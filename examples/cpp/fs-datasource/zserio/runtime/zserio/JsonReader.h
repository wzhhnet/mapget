...
template <typename ALLOC>
void BitBufferAdapter<ALLOC>::visitKey(std::string_view key)
{
...
template <typename ALLOC>
void BitBufferAdapter<ALLOC>::visitValue(std::string_view)
{
...
template <typename ALLOC>
void BytesAdapter<ALLOC>::visitKey(std::string_view key)
{
    if (m_state == VISIT_KEY)
    {
        if (key == "buffer"sv)
...
template <typename ALLOC>
void BytesAdapter<ALLOC>::visitValue(std::string_view)
{
...
template <typename ALLOC>
void CreatorAdapter<ALLOC>::visitKey(std::string_view key)
{
    if (m_objectValueAdapter)
    {
...
template <typename ALLOC>
void CreatorAdapter<ALLOC>::visitValue(std::string_view stringValue)
{
    if (m_objectValueAdapter)
    {
