...
public:
     ...
     *
     * \return Current value as an BasicAny.
     */
...
private:
    BasicString<RebindAlloc<ALLOC, char>> readContent(const ALLOC& allocator);
...
JsonToken BasicJsonTokenizer<ALLOC>::next()
{
    while (!decodeNext())
    {
        BasicString<RebindAlloc<ALLOC, char>> newContent = readContent(m_content.get_allocator());
        if (newContent.empty())
        {
...
bool BasicJsonTokenizer<ALLOC>::decodeNext()
{
    ...
    case ',':
        ...
        break;
    default:
        m_decoderResult = m_decoder.decodeValue(std::string_view(m_content.data()).substr(m_pos));
...
template <typename ALLOC>
void BasicJsonTokenizer<ALLOC>::setToken(JsonToken token, BasicAny<ALLOC>&& value)
{
    m_token = token;
