...
struct ValidationSqliteUtil
{
    ...
    {
        StringType name; /**< Column name. */
        StringType type; /**< Column SQLite data type ("INTEGER", "REAL", "TEXT" or "BLOB"). */
        bool isNotNull; /**< Is true if the column has "NOT NULL" constraint. */
        bool isPrimaryKey; /**< Is true if the column is primary key. */
    };

    using TableSchema = std::map<StringType, ColumnDescription, std::less<StringType>,
            RebindAlloc<ALLOC, std::pair<const StringType, ColumnDescription>>>;
     ...
     * \param allocator      Allocator to use for the query string composition.
     */
    static void getTableSchema(SqliteConnection& connection, std::string_view attachedDbName,
            std::string_view tableName, TableSchema& tableSchema, const ALLOC& allocator)
    {
        StringType sqlQuery(allocator);
        sqlQuery += "PRAGMA ";
        if (!attachedDbName.empty())
        ...
        while ((result = sqlite3_step(statement.get())) == SQLITE_ROW)
        {
            ...
            tableSchema.emplace(StringType(columnName, allocator),
                    ColumnDescription{
                            StringType(columnName, allocator), StringType(columnType, allocator),
     ...
     *         returns false.
     */
    static bool isColumnInTable(SqliteConnection& connection, std::string_view attachedDbName,
            std::string_view tableName, std::string_view columnName, const ALLOC& allocator)
    {
