...
template <typename T, typename ALLOC>
BasicAny<ALLOC> makeAnyBoolValue(bool value, const ALLOC& allocator)
{
    return BasicAny<ALLOC>(static_cast<T>(value), allocator);
}

template <typename T, typename U, typename ALLOC>
BasicAny<ALLOC> makeAnyBoolValue(const U& value, const ALLOC&)
{
    throw CppRuntimeException("ZserioTreeCreator: Value '") << value << "' cannot be converted to bool value!";
}

template <typename T, typename ALLOC>
BasicAny<ALLOC> makeAnyIntegralValue(bool value, const ALLOC&)
{
...
template <typename T, typename ALLOC>
BasicAny<ALLOC> makeAnyFloatingValue(bool value, const ALLOC&)
{
    throw CppRuntimeException("ZserioTreeCreator: Bool value '")
...
BasicAny<ALLOC> makeAnyStringValue(const BasicString<RebindAlloc<ALLOC, char>>& value, const ALLOC& allocator)
{
    return BasicAny<ALLOC>(value, allocator);
}

template <typename ALLOC>
BasicAny<ALLOC> makeAnyStringValue(BasicString<RebindAlloc<ALLOC, char>>&& value, const ALLOC& allocator)
{
    return BasicAny<ALLOC>(std::move(value), allocator);
}

template <typename ALLOC>
BasicAny<ALLOC> makeAnyStringValue(std::string_view value, const ALLOC& allocator)
{
    return BasicAny<ALLOC>(toString(value, allocator), allocator);
}

template <typename ALLOC>
BasicAny<ALLOC> makeAnyStringValue(const char* value, const ALLOC& allocator)
{
    return makeAnyStringValue(std::string_view(value), allocator);
}

template <typename T, typename ALLOC>
BasicAny<ALLOC> makeAnyStringValue(const T&, const ALLOC&)
{
    throw CppRuntimeException("ZserioTreeCreator: Trying to make any string value from unsupported type!");
}

template <typename ALLOC>
BasicAny<ALLOC> parseEnumStringValue(
...
template <typename ALLOC>
BasicAny<ALLOC> makeAnyEnumValue(
        std::string_view stringValue, const IBasicTypeInfo<ALLOC>& typeInfo, const ALLOC& allocator)
{
    if (!stringValue.empty())
    {
        ...
        if ((firstChar >= 'A' && firstChar <= 'Z') || (firstChar >= 'a' && firstChar <= 'z') ||
                firstChar == '_')
        {
            BasicAny<ALLOC> anyValue = parseEnumStringValue(stringValue, typeInfo, allocator);
...
template <typename ALLOC>
BasicAny<ALLOC> makeAnyEnumValue(const BasicString<RebindAlloc<ALLOC, char>>& stringValue,
        const IBasicTypeInfo<ALLOC>& typeInfo, const ALLOC& allocator)
{
    return makeAnyEnumValue(std::string_view(stringValue), typeInfo, allocator);
}
...
BasicAny<ALLOC> makeAnyEnumValue(T enumValue, const IBasicTypeInfo<ALLOC>&, const ALLOC& allocator)
{
    return BasicAny<ALLOC>(enumValue, allocator);
}

template <typename T, typename ALLOC, typename std::enable_if<!std::is_enum<T>::value, int>::type = 0>
BasicAny<ALLOC> makeAnyEnumValue(T enumRawValue, const IBasicTypeInfo<ALLOC>& typeInfo, const ALLOC& allocator)
{
...
template <typename ALLOC>
BasicAny<ALLOC> parseBitmaskNumericStringValue(
        const char* stringValue, const IBasicTypeInfo<ALLOC>& typeInfo, const ALLOC& allocator)
{
    char* pEnd = nullptr;
    errno = 0;
...
template <typename ALLOC>
BasicAny<ALLOC> makeAnyBitmaskValue(
        std::string_view stringValue, const IBasicTypeInfo<ALLOC>& typeInfo, const ALLOC& allocator)
{
    if (!stringValue.empty())
    {
        ...
        if ((firstChar >= 'A' && firstChar <= 'Z') || (firstChar >= 'a' && firstChar <= 'z') ||
                firstChar == '_')
        {
            BasicAny<ALLOC> anyValue = parseBitmaskStringValue(stringValue, typeInfo, allocator);
            if (anyValue.hasValue())
            {
                return anyValue;
            }
        ...
        else if (firstChar >= '0' && firstChar <= '9') // bitmask can be only unsigned
        {
            ...
            const BasicString<RebindAlloc<ALLOC, char>> numericStringValue = toString(stringValue, allocator);
            BasicAny<ALLOC> anyValue =
...
template <typename ALLOC>
BasicAny<ALLOC> makeAnyBitmaskValue(const BasicString<RebindAlloc<ALLOC, char>>& stringValue,
        const IBasicTypeInfo<ALLOC>& typeInfo, const ALLOC& allocator)
{
    return makeAnyBitmaskValue(std::string_view(stringValue), typeInfo, allocator);
}
...
BasicAny<ALLOC> makeAnyBitmaskValue(T bitmaskValue, const IBasicTypeInfo<ALLOC>&, const ALLOC& allocator)
{
    return BasicAny<ALLOC>(bitmaskValue, allocator);
}
...
BasicAny<ALLOC> makeAnyValue(const IBasicTypeInfo<ALLOC>& typeInfo, T&& value, const ALLOC& allocator)
{
    ...
    default:
        return BasicAny<ALLOC>(std::forward<T>(value), allocator);
    }
}

// overload for values which are already in BasicAny
template <typename ALLOC>
BasicAny<ALLOC> makeAnyValue(const IBasicTypeInfo<ALLOC>&, BasicAny<ALLOC>&& anyValue, const ALLOC&)
{
...
private:
    ...
    using BasicFieldInfoRef = std::reference_wrapper<const BasicFieldInfo<ALLOC>>;

    const IBasicTypeInfo<ALLOC>& getTypeInfo() const;
    const BasicFieldInfo<ALLOC>& findFieldInfo(
            const IBasicTypeInfo<ALLOC>& typeInfo, std::string_view name) const;

    template <typename T>
    BasicAny<ALLOC> makeAnyValue(const IBasicTypeInfo<ALLOC>& typeInfo, T&& value) const;
...
template <typename ALLOC>
void BasicZserioTreeCreator<ALLOC>::beginArray(const BasicString<RebindAlloc<ALLOC, char>>& name)
{
...
template <typename ALLOC>
void BasicZserioTreeCreator<ALLOC>::beginCompound(const BasicString<RebindAlloc<ALLOC, char>>& name)
{
    if (m_state != detail::CreatorState::IN_COMPOUND)
    {
...
template <typename ALLOC>
template <typename T>
void BasicZserioTreeCreator<ALLOC>::setValue(const BasicString<RebindAlloc<ALLOC, char>>& name, T&& value)
{
    if (m_state != detail::CreatorState::IN_COMPOUND)
    {
...
template <typename ALLOC>
void BasicZserioTreeCreator<ALLOC>::setValue(
        const BasicString<RebindAlloc<ALLOC, char>>& name, std::nullptr_t nullValue)
{
    if (m_state != detail::CreatorState::IN_COMPOUND)
    {
    ...
    if (fieldInfo.isOptional)
    {
        ...
        m_valueStack.back()->setField(fieldInfo.schemaName, BasicAny<ALLOC>(nullValue, get_allocator()));
    }
    else
    {
...
template <typename ALLOC>
const IBasicTypeInfo<ALLOC>& BasicZserioTreeCreator<ALLOC>::getFieldType(
        const BasicString<RebindAlloc<ALLOC, char>>& name) const
{
    if (m_state != detail::CreatorState::IN_COMPOUND)
    {
...
template <typename ALLOC>
template <typename T>
BasicAny<ALLOC> BasicZserioTreeCreator<ALLOC>::makeAnyValue(
