...
void BasicJsonParser<ALLOC>::parseMember()
{
    ...
    const BasicAny<ALLOC>& key = m_tokenizer.getValue();
    if (!key.template isType<BasicString<RebindAlloc<ALLOC, char>>>())
    {
        throw JsonParserException("JsonParser:")
                << getLine() << ":" << getColumn() << ": Key must be a string value!";
    }
    m_observer.visitKey(key.template get<BasicString<RebindAlloc<ALLOC, char>>>());
    m_tokenizer.next();
...
{
    const BasicAny<ALLOC>& value = m_tokenizer.getValue();
    ...
    {
        m_observer.visitValue(value.template get<BasicString<RebindAlloc<ALLOC, char>>>());
    }
}
