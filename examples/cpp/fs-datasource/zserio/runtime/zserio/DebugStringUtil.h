        ...
        const T& object, std::ostream& stream, uint8_t indent, WALK_FILTER&& walkFilter, const ALLOC& allocator)
{
    ...
    walker.walk(reflectable(object, allocator));
}
...
{
    using ReflectableDataPtr = IBasicReflectableDataPtr<ALLOC>;
};
...
template <typename T, typename WALK_FILTER, typename ALLOC = typename T::allocator_type,
        typename std::enable_if<
                ...
                int>::type = 0>
BasicString<RebindAlloc<ALLOC, char>> toJsonString(
        const T& object, WALK_FILTER&& walkFilter, const ALLOC& allocator = ALLOC())
{
...
{
    return std::move(ReflectableUtil::getValue<T, ALLOC>(fromJsonStream(typeInfo<T>(), is, allocator)));
}
 ...
 *     IReflectableDataPtr reflectable = fromJsonString(typeInfo<SomeZserioObject>(), str);
 *
 *     reflectable->getField("fieldU32")->getUInt32(); // 13
 * \endcode
 *
...
{
    return std::move(ReflectableUtil::getValue<T, ALLOC>(fromJsonString(typeInfo<T>(), json, allocator)));
}
